...
export const StaffDashboard: React.FC<StaffDashboardProps> = ({ onOpenAddListing }) => {
  ...
  const activeBreak = todayAttendanceRecord?.breaks.find(b => !b.endTime);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-gradient-to-r from-slate-900 via-slate-850 to-indigo-950 text-white p-5 sm:p-8 rounded-2xl sm:rounded-3xl border border-slate-800 shadow-xl">
        ...
        <div className="flex items-center gap-2.5 sm:gap-3 shrink-0 flex-wrap">
          <button
            ...
            className="flex-1 sm:flex-initial px-3.5 sm:px-4 py-2.5 bg-amber-500 hover:bg-amber-400 text-slate-950 text-xs font-bold uppercase tracking-wider rounded-xl shadow-md transition-all flex items-center justify-center gap-2 cursor-pointer"
          >
      ...
      <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold shrink-0 ${
            ...
            {activeBreak ? (
              <Coffee className="w-5 h-5 animate-pulse text-amber-600" />
            ) : (
          ...
          <div>
            ...
            <div className="text-xs text-slate-700 font-medium mt-0.5">
              {todayAttendanceRecord?.clockIn ? (
                <span>
                  In at {new Date(todayAttendanceRecord.clockIn).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                  {todayAttendanceRecord.clockOut && ` • Out at ${new Date(todayAttendanceRecord.clockOut).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`}
                  {' • '}{todayAttendanceRecord.breaks.length}/2 breaks taken
                </span>
              ) : (
                <span>You have not clocked in for today's work shift yet.</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0 flex-wrap">
          {!todayAttendanceRecord?.clockIn ? (
            ...
            <button
              onClick={() => endBreak()}
              className="px-4 py-2 bg-amber-500 hover:bg-amber-400 text-slate-950 text-xs font-bold rounded-xl flex items-center gap-1.5 shadow-sm animate-pulse transition-colors cursor-pointer"
            >
            ...
            <>
              {!isBreakLimitReached && (
                <button
                  onClick={() => startBreak()}
                  className="px-3.5 py-2 bg-amber-50 hover:bg-amber-100 border border-amber-300 text-amber-900 text-xs font-bold rounded-xl flex items-center gap-1.5 transition-colors cursor-pointer"
                >
                  <Coffee className="w-3.5 h-3.5 text-amber-600" />
                  <span>Start Break ({totalBreaksTaken + 1}/2)</span>
                </button>
              )}
              ...
              <button
                ...
                className="px-3.5 py-2 bg-rose-50 hover:bg-rose-100 border border-rose-300 text-rose-800 text-xs font-bold rounded-xl flex items-center gap-1.5 transition-colors cursor-pointer"
              >
                <Square className="w-3.5 h-3.5 fill-rose-800" />
                <span>Clock Out</span>
              </button>
            </>
          ) : null}
          ...
          <button
            onClick={() => setActiveStaffTab('attendance')}
            className="px-3 py-2 text-xs font-bold text-slate-700 hover:text-slate-950 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors flex items-center gap-1 cursor-pointer"
          >
            <span>Full Log & Roster</span>
